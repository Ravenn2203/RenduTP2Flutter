...
class _QuizzPageState extends State<QuizzPage> {
  ...
  Container _getImage() {
    return Container(
      ...
      child: BlocBuilder<QuizzCubit, QuizzState>(builder: (context, state) {
        if (state is QuizzInitial || state is QuizzColorsLoaded) {
          return Image.asset(
        ...
        } else if (state is QuizzNextQuestionLoaded) {
          return Image.asset(
            BlocProvider.of<QuizzCubit>(context).question?.imagePath ?? "",
            height: 180,
            width: 450,
  ...
  Card _getQuestionCard() {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: BlocBuilder<QuizzCubit, QuizzState>(builder: (context, state) {
          if (state is QuizzInitial || state is QuizzColorsLoaded) {
            return Text(BlocProvider.of<QuizzCubit>(context).question?.questionText ??
                "",
                style: TextStyle(
                  color: Colors.white,
                  letterSpacing: 2.0,
          ...
          onPressed: () {
            if (Provider.of<QuizzCubit>(context, listen: false).numQuestion ==
              ...
              if (Provider.of<QuizzCubit>(context, listen: false).aRepondu) {
                context
                    .read<QuizzCubit>()
                    .prochaineQuestion(Colors.grey, Colors.grey);
  ...
  bool _checkAnswer(bool userChoice, BuildContext context) {
    bool response = Provider.of<QuizzCubit>(context, listen: false).question.isCorrect;
